# Copied from https://github.com/yi-sun/circom-pairing/blob/107c316223a08ac577522c54edd81f0fc4c03130/python/bn254.ipynb

# need to run `pip install py_ecc`
...
def create_circom_input():
    n = 43
    k = 6

    inputParameters = convert_vkey(n, k)
    proofParameters = convert_proof(n, k)
    pubParameters = convert_public_params(n, k)

    fullCircomInput = {**inputParameters, **proofParameters, **pubParameters}

    with open('./full-circom-input.json', 'w') as outfile:
